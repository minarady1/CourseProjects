...
//char filename [] = "EXP10.TXT";
/*
EXP 21: using most robust communication inside and outsid CRAN building
EXP 22: using most robust communication from charmois
EXP 23: using SF 10 from charmois
*/
char logfile [] = "EXP23R.TXT";
...
void loop() 
{
  ...
  if (error == 0)
  {
    Log("Received frame");
    USB.print(RTC.getEpochTime());
    USB.println(" :Received Packet, Will blink");
    Utils.blinkLEDs(500);
    Utils.blinkLEDs(500);
    USB.print(RTC.getEpochTime());
    USB.println(" :End blink, will store");

    USB.println((char*) LoRaWAN._buffer);
    char data[LoRaWAN._length];
    sprintf( data,"%s", LoRaWAN._buffer);
    USB.println(data);
    uint16_t len = LoRaWAN._length;

    char toz [10] ;
    USB.printf("store counter:%d",store_counter);
    //if (store_counter%8 ==0)
    if (true)
    {
    ...
    SD.append(logfile, _pre);
    SD.append(logfile,  ",");
    SD.append(logfile, _post);
    SD.append(logfile,  ",");
    
    sd_answer = SD.appendln(logfile, (uint8_t*)data,len);
    USB.printf("sd answer: %d",sd_answer);
    USB.print(RTC.getEpochTime());
    USB.println(" :End Storage");
...
{ 
 ...
 //////////////////////////////////////////////
  // 3. Set/Get Radio Preamble
  //////////////////////////////////////////////

  // Set preamble
  e = LoRaWAN.setRadioPreamble(preamble);

  // Check status
  if (e == 0)
  {
    USB.println(F("3.1. Set Radio preamble OK"));
  }
  else
  {
    USB.print(F("3.1. Set Radio preamble error = "));
    USB.println(e, DEC);
    status = 1;
  }
  ...
  if (e == 0)
  {
    USB.print(F("3.2. Get Radio Preamble OK. "));
    USB.print(F("Preamble: "));
    USB.println(LoRaWAN._preambleLength);
  }
  else
  {
    USB.print(F("3.2. Get Radio Preamble error = "));
    USB.println(e, DEC);
    status = 1;
  }
  USB.println(F("-------------------------------------------------------"));
