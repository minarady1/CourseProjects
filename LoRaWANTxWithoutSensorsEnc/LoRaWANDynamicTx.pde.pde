...
void loop()
{
  ...
  //CO2.ON();
  USB.print(RTC.getEpochTime());
  USB.println(" :Sensor On complete. Sleep 1 Start");
  ...
  dtostrf(PWR.getBatteryVolts(), 12, 10, toz);
  frame.addSensor(SENSOR_BAT, PWR.getBatteryVolts());
  ...
  /*
    call encrypt on uint8_t buffer
    get encrypted message in uint8_t results
    pass uint8_t msg to hex2str for conversion along iwth its length
  */
